...
fn solve(data: []const u8, _: std.mem.Allocator) !usize {
    ...
    while (time_itr.next()) |time_str| {
        ...
        time *= std.math.pow(usize, 10, time_str.len);
        time += try std.fmt.parseUnsigned(usize, time_str, 10);
    }
    ...
    while (dist_itr.next()) |dist_str| {
        ...
        dist *= std.math.pow(usize, 10, dist_str.len);
        dist += try std.fmt.parseUnsigned(usize, dist_str, 10);
    }

    // Who needs maths right...?
    for (1..time) |t| {
        var d = (time - t) * t;
        if (d > dist) {
            result += 1;
...
test "Example" {
    ...
    try std.testing.expectEqual(@as(@TypeOf(result), 71503), result);
}
