...
pub fn main() !void {
    ...
    defer _ = gpa.deinit();
    var result = try solve(data, gpa.allocator(), 1_000_000);
...
fn solve(data: []const u8, child_allocator: std.mem.Allocator, expansion: usize) !usize {
    var result: usize = 0;
...
test "Example 1" {
    ...
    var result = try solve(data, std.testing.allocator, 10);

    try std.testing.expectEqual(@as(@TypeOf(result), 1030), result);
}
...
test "Example 2" {
    ...
    var result = try solve(data, std.testing.allocator, 100);

    try std.testing.expectEqual(@as(@TypeOf(result), 8410), result);
}
